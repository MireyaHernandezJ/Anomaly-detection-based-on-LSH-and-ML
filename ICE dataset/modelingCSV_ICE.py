...
path='/home/mireya/Documentos/Datasets-IoT/ICEdataset.csv'
df=pd.read_csv(path,low_memory=False)

#SECTION - Pre-processing steps

#SECTION - Label encoder:
#SECTION - identify categorical columns
categorical_cols=df.select_dtypes(include=['object']).columns.tolist()
#SECTION - Creat a LabelEnconder object to transform categorical data to numeric data
Cat2Num=preprocessing.LabelEncoder()
#SECTION - Loop over categorcal columns and make the transformation
for col in categorical_cols:
    df[col]=Cat2Num.fit_transform(df[col].astype(str))
    
#SECTION - Replace missing values with 0:
df=df.fillna(0)

#!SECTION - extract labels and drop labels columns from dataset
labels=df['Label']
...
data="CSV_ICE-TrainTest-Binary"
hash="" #tlsh or nilsimsa
windowtime="10s"#["5s","10s","15s","20s"]
